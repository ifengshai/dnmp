...
class Process extends Backend
{
    ...
    {
        $zeeloolde = new ZeeloolDe();
        $order = new NewOrder();
        $zeeloolde->where('coupon_code is not null')->field('entity_id,coupon_code,coupon_rule_name')->chunk(10000, function ($row) use ($order) {
            $row = collection($row)->toArray();
            foreach ($row as $k => $v) {
                $order->where(['entity_id' => $v['entity_id'], 'site' => 10])
                    ->update(['coupon_code' => $v['coupon_code'], 'coupon_rule_name' => $v['coupon_rule_name']]);
                echo  $v['entity_id'] . "\n";
                usleep(100000);
            }
        });
