...
class Process extends Backend
{
    ...
    public function test011()
    {
        ...
        $site = 'voogueme';
        for ($i = 0; $i < 24; $i++) {
            $start = '2021-08-21T' . $i . ':00:00Z';
            $end = '2021-08-21T' . $i . ':59:59Z';
            try {
                $this->asyncTicketHttps($type, $site, $start, $end);
                usleep(10000);
            } catch (Exception $e) {
    ...
    public function jianuoWorkOrderList()
    {
        ...
        $export_str = ['订单号', '订单金额', '工单问题类型', '退款金额', '措施状态', '补发订单号', '补发订单号状态'];
    ...
    public function add_order_coupon()
    {
        ...
        $nihao->where('coupon_code is not null')->field('entity_id,coupon_code,coupon_rule_name')->chunk(10000, function ($row) use ($order) {
            ...
            foreach ($row as $k => $v) {
                ...
                if ($res !== false) {
                    echo $v['entity_id'] . "->推送成功" . PHP_EOL;
                } else {
                    echo $v['entity_id'] . "->推送失败" . PHP_EOL;
                }
            }
        });
        echo "ok";
     ...
     */
    public function process_goods_name()
    {
        $orderOption = new NewOrderItemOption();
        Db::connect('database.db_nihao')->table('order_items')->chunk(10000, function ($data) use ($orderOption) {
            foreach ($data as $k => $v) {
                $orderOption->where(['item_id' => $v['id'], 'site' => 3, 'magento_order_id' => $v['order_id']])->update(['name' => $v['goods_name']]);
                echo $v['id'] . "\n";
            }
