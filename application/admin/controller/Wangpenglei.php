...
class Wangpenglei extends Backend
{
    ...
    public function derver_data()
    {
        ...
        $this->itemplatformsku = new \app\admin\model\itemmanage\ItemPlatformSku;
        $this->item = new \app\admin\model\itemmanage\Item;
        $sql = "select sku,site from fa_sku_sales_num where site in (1,2,3) and createtime BETWEEN '2020-06-01 00:00:00' and '2020-08-31 23:59:59' GROUP BY sku,site";
        $list = db()->query($sql);
        foreach ($list as $k => $v) {
            if ($v['site'] == 1) {
            ...
            $map['a.distribution_status'] = ['<>', 0]; //排除取消状态
            $map['b.created_at'] = ['between', [strtotime('2020-06-01 00:00:00'), strtotime('2020-08-31 23:59:59')]]; //时间节点
            $map['b.site'] = $v['site'];
        ...
        $headlist = ['sku', '站点', '销售额'];
        Excel::writeCsv($list, $headlist, '6月份sku销量');
        die;
    }
