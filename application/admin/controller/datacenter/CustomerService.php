...
class CustomerService extends Backend
{
    ...
    public function detail()
    {
        ...
        if ($this->request->isAjax()) {
            ...
            if ($params['time']) {
                ...
                $map['complete_time'] = ['between', [date('Y-m-d 00:00:00'), date('Y-m-d H:i:s', time())]];
            }
            ...
            //问题措施比统计
            $problem_step_data = $this->get_problem_step_data($platform,$map,1);
    ...
    public function get_workorder_data($platform, $map)
    {
        ...
        foreach($problem_arr as $k=> $v){
            //问题大分类的统计 
            $result['problem_type'][$k] = $this->model->where($where)->where($map)->where('problem_type_id','in',$v)->count('id');           
        }
        //所有完成的work_id
