...
class Operationalreport extends Backend{
    ...
    public function platformOrderInfo($platform,$map,$itemMap)
    {
        ...
        //眼镜动销数
        $frame_in_print_num        = $model->table('sales_flat_order_item m')->join('sales_flat_order o','m.order_id=o.entity_id','left')->where($whereItem)->where($itemMap)->where('m.sku','in',$frame_sku)->distinct(true)->field('m.skus')->count('m.skus');
        //眼镜总共的数量
