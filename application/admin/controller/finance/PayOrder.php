...
class PayOrder extends Backend
{
    ...
    public function pay($ids = '')
    {
        ...
        if ($this->request->isAjax()) {
            ...
            $id = $params['id'];
            //更改状态
            $this->payorder->where('id', $id)->update(['status' => 4, 'pay_time' => time(), 'pay_person' => session('admin.nickname'), 'pay_remarks' => $params['pay_remarks']]);
            //获取付款单下所有的采购单id
            ...
            foreach ($purchase_order_ids as $v) {
                ...
                if ($batch_count == $pay_batch_count) {
                    ...
                    /**************************************计算成本冲减start****************************************/
                    $result = array();
                    $purchase_order = $this->purchase_item->alias('i')->join('fa_purchase_order o',
                        'i.purchase_id=o.id')->where('i.purchase_id',
                        $v)->field('purchase_price,actual_purchase_price,i.sku')->find();
                    //实际采购成本和预估成本不一致，冲减差值
                    if ($purchase_order['purchase_price'] != $purchase_order['actual_purchase_price']) {
                        ...
                        if ($out_count1) {
                            //订单出库
                            $order = $this->item->where('purchase_id', $v)->where('item_order_number', '<>',
                                '')->where('sku', $purchase_order['sku'])->where('library_status', 2)->select();
                            $result1 = array();
                            ...
                            foreach ($result1 as $rr1 => $ss1) {
                                //获取成本核算中的订单数据
                                $cost_order_info = $this->financecost->where([
                                    'order_number' => $rr1,
                                    'type' => 2,
                                    'bill_type' => 8
                                ])->order('id desc')->find();
                                ...
                                $arr1['frame_cost'] = $cost_order_info['frame_cost'];    //镜架成本：原订单金额
                                ...
                                $arr2['frame_cost'] = round($cost_order_info['frame_cost'] + $ss1 * ($purchase_order['actual_purchase_price'] - $purchase_order['purchase_price']),
                                    2);    //镜架成本：（实际单价-预估）*数量+原订单金额
                        ...
                        if ($out_count2) {
                            //出库单出库
                            $outorder = $this->outstockItem->alias('i')->join('fa_out_stock s', 's.id=i.out_stock_id',
                                'left')->where('s.purchase_id', $v)->where('status', 2)->where('i.sku',
                                $purchase_order['sku'])->group('s.out_stock_number')->field('s.id,s.out_stock_number,sum(i.out_stock_num) count')->select();
                            foreach ($outorder as $rr2 => $ss2) {
                                //如果有出库数据，需要添加冲减暂估结算金额和增加成本核算数据
                                $arr3['type'] = 2;   //类型：成本
                                $arr3['bill_type'] = 11;    //单据类型：暂估结算金额
                                $arr3['frame_cost'] = round($ss2['count'] * $purchase_order['purchase_price'],
                                    2);    //镜架成本：剩余预估单价*剩余数量
    ...
    public function getPayinfo($ids = 0)
    {
        ...
        //结算信息
        $settle = $this->statement
            ->alias('s')
            ->join('fa_finance_statement_item i', 'i.statement_id=s.id', 'left')
            ->join('fa_purchase_order a', 'i.purchase_id=a.id', 'left')
            ->field('s.finance_purcahse_id,i.id,i.purchase_number,i.purchase_batch_id,i.now_before_total,i.instock_total,i.deduction_total,i.deduction_total,i.deduction_reason,i.wait_statement_total,i.purchase_id,a.purchase_type,a.create_person,a.1688_number,a.pay_type')
            ->where($settle_where)->select();
        $total1 = 0;  //结算待结算金额合计
        $count1 = 0;
        ...
        //预付信息
        $prepay = $this->financepurchase->alias('p')->join('fa_purchase_order o', 'o.id=p.purchase_id', 'left')->field('p.id,o.purchase_number,o.purchase_total,p.pay_rate,p.pay_grand_total,p.pay_type,p.purchase_id,o.purchase_type,o.create_person,o.1688_number')->where('p.pay_type', 'in', '1,2')->where('p.id', 'in', $ids)->where('p.supplier_id', $supplier_id)->select();
        $total2 = 0;  //预付预付款金额合计
        $count2 = 0;
        foreach ($prepay as $k => $v) {
    ...
    public function print($ids = null)
    {
        ...
        $prepay = $this->payorder_item
            ->alias('a')
            ->field('a.*,b.create_person,b.purchase_type,b.1688_number,b.purchase_number,b.purchase_name,b.purchase_total')
            ->where(['pay_id' => $ids])
            ->where('a.pay_type', '<>', 3)
            ->join(['fa_purchase_order' => 'b'], 'a.purchase_order_id=b.id')
            ->select();
        ...
        foreach ($prepay as $k1 => $v1) {
            ...
            $pay_rate = $v1['pay_rate'] * 100;
            $prepay[$k1]['pay_rate'] = $pay_rate;
            $prepay[$k1]['category_name'] = $category_name;
            $total2 += $v1['pay_grand_total'];
            $count2++;
        }
        $total = $total1 + $total2;
        $supplier = $this->supplier->where('id', $pay_order['supply_id'])->field('id,remark,supplier_name,currency,period,opening_bank,bank_account,recipient_name')->find();
        /***********end***************/
        $this->assign(compact('pay_order', 'supplier', 'settle', 'prepay', 'total1', 'total2', 'total', 'count1', 'count2'));
