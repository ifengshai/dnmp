    ...
    public function index()
    {
        ...
        if ($this->request->isAjax()) {
            ...
            foreach ($list as $key=>$item){
                $i++;
                $list[$key]['id'] = $i;
                $prices = $this->item->alias('i')
                    ->join('fa_purchase_order_item p','p.purchase_id=i.purchase_id')
                    ->join('fa_purchase_order o','o.id=i.purchase_id')
                    ->where('i.sku',$item['sku'])
                    ->where('i.library_status',1)
                    ->where("i.item_order_number=''")
                    ->where('p.sku',$item['sku'])
                    ->field('i.id,actual_purchase_price,o.purchase_total,purchase_num')
                    ->select();
                $amount = 0;
        ...
        //库存总金额
        $purchase = $this->item->alias('i')
            ->join('fa_purchase_order_item p','p.purchase_id=i.purchase_id and p.sku=i.sku')
            ->join('fa_purchase_order o','o.id=i.purchase_id')
            ->where('i.sku','<>','')
            ->where("i.item_order_number=''")
            ->where('i.library_status',1)
            ->field('sum(purchase_price) as purchase_price,sum(actual_purchase_price) as actual_purchase_price')
            ->group('p.purchase_order_number')
            ->select();
        $amount = 0;
