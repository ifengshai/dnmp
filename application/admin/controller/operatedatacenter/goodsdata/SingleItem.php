...
class SingleItem extends Backend
{
    ...
    public function sku_sales_data_line()
    {
        if ($this->request->isAjax()) {
            ...
            $same_where['sku'] = ['=', $sku];
            $recent_day_num = Db::name('datacenter_sku_day')->where($same_where)->column('order_num', 'day_date');
            $recent_day_now = Db::name('datacenter_sku_day')->where($same_where)->column('now_pricce', 'day_date');
    ...
    public function sku_sales_data_bar()
    {
        if ($this->request->isAjax()) {
            ...
            $same_where['site'] = ['=', $site];
            $same_where['sku'] = ['=', $sku];
            $recent_30_day = Db::name('datacenter_sku_day')->where($same_where)->column('order_num', 'day_date');
