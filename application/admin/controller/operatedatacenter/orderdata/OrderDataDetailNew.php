            ...
            if ($site == 3 || $site == 5) {
                ...
                if ($site == 3) {
                    $list = $order_model->table('orders')->alias('o')
                        ...
                        ->order($sort, $order)
                        ->limit($offset, $limit)
                        ->field('o.id as entity_id,o.order_no as increment_id,o.created_at,o.code as coupon_code,o.coupon_id as discount_coupon_id,o.order_type,o.base_actual_payment as base_grand_total,o.base_freight_price as base_shipping_amount,o.status,o.source as store_id,o.freight_type as shipping_method,o.payer_email as customer_email,o.user_id as customer_id,o.base_discounts_price as base_discount_amount,o.payment_time,d.firstname,d.lastname,c.email as register_email,c.created_at as register_time,d.country,d.telephone,o.payment_type as payment_method,c.group as group_id')
                        ->select();
        ...
        for ($i = 0; $i < intval($total_export_count / $pre_count) + 1; $i++) {
            ...
            foreach ($list as &$val) {
                ...
                if (in_array('status', $column_name)) {
                    $order_node = Db::name('order_node')->where('order_id', $val['entity_id'])->where('site', $site)->value('node_type');
                    if ($order_node == 7) {
                        $order_shipping_status = '已发货';
                    } elseif ($order_node == 8 && $order_node == 10) {
                        $order_shipping_status = '运输途中';
