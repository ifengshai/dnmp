    ...
    public function user_order_pie()
    {
        if ($this->request->isAjax()) {
            ...
            if ($cache_data) {
                ...
                $map_where['o.status'] = $order_where['status'] = ['in', ['free_processing', 'processing', 'complete', 'paypal_reversed', 'payment_review', 'paypal_canceled_reversal','delivered','delivery']];
