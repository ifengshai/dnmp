...
class Distribution extends Backend
{
    ...
    public function batch_export_xls()
    {
        ...
        foreach ($list as $key => &$value) {
            ...
            $data[$value['increment_id']]['item_order'][$key]['prismcheck'] = $value['prismcheck'];
...
//            ->setCellValue("AA1", "ED");
        ...
        foreach ($data as $key => &$value) {
            ...
            foreach ($value['item_order'] as $k => $v) {
                ...
                $spreadsheet->getActiveSheet()->setCellValue("T" . ($cat), $ARd); //Prism
                $spreadsheet->getActiveSheet()->setCellValue("T" . ($cat + 1), $ALd); //Prism
                ...
                $spreadsheet->getActiveSheet()->setCellValue("W" . ($cat), $edRd); //Prism
                $spreadsheet->getActiveSheet()->setCellValue("W" . ($cat + 1), $edLd); //Prism
...
//                $spreadsheet->getActiveSheet()->setCellValue("AA" . ($cat + 1), number_format($ed,2,'.','')); //Prism
    ...
    public function batch_print_label()
    {
        ...
        $change_lens = $this->_work_order_change_sku
            ...
            ->column('a.od_sph,a.od_cyl,a.od_axis,a.od_add,a.pd_r,a.od_pv,a.od_bd,a.od_pv_r,a.od_bd_r,a.os_sph,a.os_cyl,a.os_axis,a.os_add,a.pd_l,a.os_pv,a.os_bd,a.os_pv_r,a.os_bd_r,a.lens_number,a.recipe_type as prescription_type,a.web_lens_name,a.prismcheck', 'a.item_order_number');
        if ($change_lens) {
            foreach ($change_lens as $key => $val) {
                if ($val['pd_l'] && $val['pd_r']) {
                    $change_lens[$key]['pd'] = '';
