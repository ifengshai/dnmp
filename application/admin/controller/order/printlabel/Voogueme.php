...
where cpev.attribute_id in(161,163,164) and cpev.store_id=0 and cpev.entity_id=$product_id";
            $resultList = Db::connect('database.db_voogueme_online')->query($querySql);
            if ($resultList) {
                $result = array();
...
order by sfoi.order_id desc;";
        $resultList = Db::connect('database.db_voogueme_online')->query($processing_order_querySql);
        // dump($resultList);
...
order by sfoi.order_id desc;";
            $processing_order_list = Db::connect('database.db_voogueme_online')->query($processing_order_querySql);
