 ...
 * @icon fa fa-circle-o
 */
class ZeeloolEs extends Backend
{
    ...
    {
        parent::_initialize();
        $this->model = new \app\admin\model\order\printlabel\ZeeloolEs;
    }
    ...
    public function detail($ids = null)
    {
        ...
        //查询订单详情
        $result = ZeeloolEsPrescriptionDetailHelper::get_one_by_entity_id($ids);
            ...
            if ($status == 1) {
                ...
                if ($infoRes) {
                    foreach ($infoRes as $k => $v) {
                        //sku转换
                        $trueSku = $ItemPlatformSku->getTrueSku(trim($v['change_sku']), 9);
                        if (!$trueSku) {
                ...
                foreach ($list as $k => &$v) {
                    ...
                    //转仓库SKU
                    $trueSku = $ItemPlatformSku->getTrueSku(trim($v['sku']), 9);
                    if (!$trueSku) {
...
where cpev.attribute_id in(161,163,164) and cpev.store_id=0 and cpev.entity_id=$product_id";
            $resultList = Db::connect('database.db_zeelool_es')->query($querySql);
            if ($resultList) {
                $result = array();
    ...
    public function batch_export_xls()
    {
        ...
        foreach ($finalResult as $key => $value) {
            ...
            //网站SKU转换仓库SKU
            $sku = $ItemPlatformSku->getTrueSku($value['sku'], 9);
...
order by NUM asc;";
            $processing_order_list = Db::connect('database.db_zeelool_es')->query($processing_order_querySql);
...
EOF;
            ...
            foreach ($processing_order_list as $processing_key => $processing_value) {
                if ($temp_increment_id != $processing_value['increment_id']) {
                    ...
                    $fileName = ROOT_PATH . "public" . DS . "uploads" . DS . "printOrder" . DS . "zeelool_es" . DS . "$date" . DS . "$temp_increment_id.png";
                    // dump($fileName);
                    $dir = ROOT_PATH . "public" . DS . "uploads" . DS . "printOrder" . DS . "zeelool_es" . DS . "$date";
                    if (!file_exists($dir)) {
                        mkdir($dir, 0777, true);
                        // echo '创建文件夹$dir成功';
                    } else {
                    ...
                    $img_url = "/uploads/printOrder/zeelool_es/$date/$temp_increment_id.png";
                    //生成条形码
