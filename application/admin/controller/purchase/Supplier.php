...
class Supplier extends Backend
{
        ...
        try {
            ...
            if ($has_admin_id) {
                ...
                foreach ($insert as &$val) {
                    ...
                    //判断供应商类型
                    if ($val['supplier_type'] == '镜片') {
                        $val['supplier_type'] = 1;
                    } elseif ($val['supplier_type'] == '镜架') {
                        $val['supplier_type'] = 2;
                    } elseif ($val['supplier_type'] == '眼镜盒') {
                        $val['supplier_type'] = 3;
    ...
    {
        //查询所有SKU
        $data = db('zeelool_product')->alias('a')->where('a.is_visable=1')->join(['fa_supplier' => 'b'], 'a.supplier_id=b.id')->select();
