 ...
 * @icon fa fa-circle-o
 */
class SaleAfterIssue extends Backend
{
    
    /**
     * SaleAfterIssue模型对象
     * @var \app\admin\model\saleAfterManage\SaleAfterIssue
     */
    ...
    {
        if ($ids) {
            $nextIds = $this->model->getLowerIssue($ids);
            if($nextIds){
                $ids = $ids.','.$nextIds;
            }
            $pk = $this->model->getPk();
        ...
        if ($this->request->isPost()) {
            ...
            if ($params) {
                ...
                if(0 == $params['pid']){
                    ...
                    $pidLevel = $this->model->findLevel($params['pid']);
                    $params['level'] = $pidLevel+1;
                }
                if(3<$params['level']){
                    $this->error('等级不能超过3级,请重新选择');
                }
