...
class WorkOrderList extends Backend
{
    ...
    public function detail($ids = null)
    {
        ...
        if ($row['work_platform'] == 1 && $row['replenish_money']) {
            $url = config('url.new_zeelool_url') . 'price_difference?customer_email=' . $row['email'] . '&origin_order_number=' . $row['platform_order'] . '&order_amount=' . $row['replenish_money'] . '&sign='. $row->id;
