...
class TrackReg extends Backend
{
    ...
    public function get_days_sales_num()
    {
        ...
        $list = $itemPlatformSku->field('sku,platform_type as site')->where(['outer_sku_status' => 1,'sku' => 'OM268544-03'])->select();
        $list = collection($list)->toArray();
        
        foreach ($list as $k => $v) {
            //15天日均销量
            $days15_data = $skuSalesNum->where(['sku' => 'OM268544-03', 'site' => $v['site'], 'createtime' => ['<', $date]])->field("sum(sales_num) as sales_num,count(*) as num")->limit(15)->find();
            $params['sales_num_15days'] = $days15_data->num > 0 ? round($days15_data->sales_num / $days15_data->num) : 0;
            $days90_data = $skuSalesNum->where(['sku' => 'OM268544-03', 'site' => $v['site'], 'createtime' => ['<', $date]])->field("sum(sales_num) as sales_num,count(*) as num")->limit(90)->find();
            //90天日均销量
