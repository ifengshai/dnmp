...
DOC;
    ...
    {
        $tickets = $this->model->where('id','between',[30660,32660])->order('id asc')->select();
        foreach($tickets as $ticket){
    ...
    {
        $tickets = $this->model->where('id','between',[32660,34660])->order('id asc')->select();
        foreach($tickets as $ticket){
            $ticketId = $ticket->ticket_id;
            if($ticket->type == 1){
                (new Notice(request(), ['type' => 'zeelool','id' => $ticketId]))->update();
    ...
    {
        $tickets = $this->model->where('id','between',[34660,36660])->order('id asc')->select();
        foreach($tickets as $ticket){
            $ticketId = $ticket->ticket_id;
            if($ticket->type == 1){
                (new Notice(request(), ['type' => 'zeelool','id' => $ticketId]))->update();
