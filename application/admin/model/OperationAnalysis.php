...
class OperationAnalysis extends Model
{
    ...
    // 追加属性
    protected $append = [];
    protected $order_status =  "and status in ('processing','complete','creditcard_proccessing','free_processing','paypal_canceled_reversal','paypal_reversed','delivered','delivery')";
    /**
     * 通过id判断需要传递的model
     *
    ...
    public function get_today_order_num($id)
    {
        ...
        if ($id == 5) {
            ...
            //今日订单数sql
            $today_order_num_sql = "SELECT count(*) counter FROM sales_flat_order WHERE  created_at between '$date_time_start' and '$date_time_end'";
    ...
    public function get_today_register_customer($id)
    {
        ...
        if ($id == 5) {
            ...
            //今日新增注册用户数
            $today_register_customer_sql = "SELECT count(*) counter from customer_entity where created_at between '$date_time_start' and '$date_time_end'";
