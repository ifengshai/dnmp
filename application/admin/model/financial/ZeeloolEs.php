...
class ZeeloolEs extends Model
{
    ...
    {
        parent::__construct();
        $this->facebook = Fackbook::where('platform', 9)->find();
    ...
    protected function getReport($analytics, $startDate, $endDate)
    {
        ...
        // $VIEW_ID = "168154683";
        // $VIEW_ID = "172731925";
        $VIEW_ID = config('ZEELOOLES_GOOGLE_ANALYTICS_VIEW_ID');
    ...
    public function all_cost($start_time, $end_time)
    {
        ...
        foreach ($all_frame_result as $key => $value) {
            $true_sku = $this->itemPlatform->getTrueSku($value['sku'], 9);
        ...
        //求销售额、运费、毛利润
        $base_grand_total_result = Db::connect('database.db_zeelool_es')->table('sales_flat_order o')->where($whereFrame)
