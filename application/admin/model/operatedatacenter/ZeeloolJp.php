...
class ZeeloolJp extends Model
{
    ...
    public function getUserOrderData($time_str)
    {
        ...
        //用户统计
        $customer = Db::connect('database.db_zeelool_jp')
            ->table('customer_entity')
            ->field('count(*) as count,group_id')
            ->where($customerWhere)
            ->group('group_id')
            ->select();
        $customerCount = Db::connect('database.db_zeelool_jp')
            ->table('customer_entity')
            ->where($customerWhere)
            ->count();
        $order = Db::connect('database.db_zeelool_jp')
            ...
            ->where($orderWhere)
            ->select();
        $orderCount = Db::connect('database.db_zeelool_jp')
            ->table('sales_flat_order')
            ->where($customerWhere)
            ->where($orderWhere)
