...
class WorkOrderList extends Model
{
    ...
    {
        $order_field = 'id,site,base_grand_total,base_to_order_rate,payment_method as method,customer_email,customer_firstname,customer_lastname,order_type,mw_rewardpoint_discount,base_currency_code,order_currency_code,shipping_method,created_at as payment_time';
    ...
    {
        //获取地址信息
        $order_field = 'id,site,customer_email as email,firstname,lastname,order_type,country_id,region,region_id,city,street,postcode,telephone,area';
        $_new_order = new NewOrder();
        $address = $_new_order
            ->where('increment_id', $increment_id)
            ->field($order_field)
...
//        $data = Cache::get($cache_key);
        ...
        if (!$data) {
            if ($siteType == 13 || $siteType == 14) {//第三方平台接口
                $data = [];
                $data['lens_list'] = ['Plastic lenses' => 'Plastic lenses', 'Standard Eyeglass Lenses' => 'Standard Eyeglass Lenses', 'Beyond UV Blue Blockers' => 'Beyond UV Blue Blockers', 'Photochromic Lenses' => 'Photochromic Lenses', 'Color Tint' => 'Color Tint', 'Mid-Index Mirrored lenses' => 'Mid-Index Mirrored lenses', 'Polarized' => 'Polarized', 'PhotochromicBlueLightBlocking' => 'PhotochromicBlueLightBlocking', 'Night vision' => 'Night vision','WanxinDrive'=>'WanxinDrive','MingYue'=>'MingYue'];
                $data['color_list'] = [];
                $data['coating_list'] = [];
            } else if ($siteType == 3){
...
//        $data = Cache::get($cache_key);
            ...
            if (0 == $resultRecept) {
                ...
                if (0 == $resultMeasure) {
                    ...
                    (new NewOrderItemProcess)
                        ->where(['item_order_number' => ['in', $work->order_item_numbers]])
                        ->update(['is_prescription_abnormal' => 0]);
                    (new NewOrder)
                        ->where(['increment_id' => $work->platform_order, 'site' => $work->work_platform])
    ...
    {
        if ($type == 1) {
            $map['complete_time'] = ['between', [$start, $end]];
        }
        $map['work_status'] = 6;
        if ($platform != 0) {
            $map['work_platform'] = $platform;
    ...
    public function wo_bufa_percent($start, $end, $platform = 0, $type = 0)
    {
        ...
        if ($type == 1) {
            $map['z.complete_time'] = ['between', [$start, $end]];
        }
        $map['z.work_status'] = 6;
        if ($platform != 0) {
    ...
    public function get_question_type($type, $platform, $time_where)
    {
        ...
        $where['work_status'] = 6;
        $where['problem_type_id'] = ['in', $problem_ids];
        if ($platform) {
