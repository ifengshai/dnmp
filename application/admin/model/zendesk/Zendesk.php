...
class Zendesk extends Model
{
    ...
    public function worknum_situation($platform = 0,$workload_time = ''){
        ...
        $all_already_num = Db::name('zendesk_comments')->alias('c')->join('fa_zendesk z','c.zid=z.id')->where($where)->where(['c.due_id'=>['in',$admin_ids]])->select(false);
        echo $all_already_num;exit;
