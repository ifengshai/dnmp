...
class ZeeloolJpPrescriptionDetailHelper
{
	...
	*/
	public static function get_one_by_entity_id($entity_id)
	{
		if ($entity_id) {
			...
			// dump($querySql);
			$item_list = Db::connect('database.db_zeelool_jp')->query($querySql);
			// dump($item_list);
			// 如果为空，则直接返回false
			if (empty($item_list)) {
				return false;
	...
	*/
	public static function get_one_by_increment_id($increment_id)
	{
		...
		if ($increment_id) {
			$querySql = "select sfoi.original_price,sfoi.item_id,sfoi.base_discount_amount,sfoi.base_row_total,sfo.increment_id,sfo.customer_email,sfo.customer_firstname,sfo.store_id,sfo.customer_lastname,sfoi.product_options,sfoi.order_id,sfo.`status`,sfoi.sku,sfoi.qty_ordered,sfoi.name,sfo.created_at
			from sales_flat_order_item sfoi
			left join sales_flat_order sfo on sfoi.order_id=sfo.entity_id 
