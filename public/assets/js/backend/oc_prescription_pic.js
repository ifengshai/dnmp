    ...
    var Controller = {
        index: function () {
            ...
            $('.btn-batch-export-xls').click(function () {
                ...
                if (ids.length > 0) {
                    window.open(Config.moduleurl + '/oc_prescription_pic/batch_export_xls?ids=' + ids, '_blank');
                } else {
                    var options = table.bootstrapTable('getOptions');
                    var search = options.queryParams({});
                    var filter = search.filter;
                    var op = search.op;
                    window.open(Config.moduleurl + '/oc_prescription_pic/batch_export_xls?filter=' + filter + '&op=' + op + '&label=' + Config.label, '_blank');
                }
