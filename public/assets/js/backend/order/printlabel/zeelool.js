define(['jquery', 'bootstrap', 'backend', 'table', 'form', 'bootstrap-table-jump-to', 'template'], function ($, undefined, Backend, Table, Form, undefined, Template) {
    ...
    var Controller = {
        index: function () {
            ...
            $('.btn-batch-export-xls').click(function () {
                var ids = Table.api.selectedids(table);
                if (ids.length > 0) {
                    window.open(Config.moduleurl + '/order/printlabel/zeelool/batch_export_xls?id_params=' + ids, '_blank');
                } else {
                    var options = table.bootstrapTable('getOptions');
                    var search = options.queryParams({});
                    var filter = search.filter;
                    var op = search.op;
                    window.open(Config.moduleurl + '/order/printlabel/zeelool/batch_export_xls?filter=' + filter + '&op=' + op, '_blank');
                }
